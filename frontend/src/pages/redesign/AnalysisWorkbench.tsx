...
export function AnalysisWorkbench() {
  ...
  return <><PageHeader eyebrow="Understand your signals" title="Analysis" description="Explore relationships with reference SOC, or prepare a leakage-safe selection."/>
    <fieldset className="mb-8"><legend className="font-medium mb-3">Analysis mode</legend><div className="grid md:grid-cols-2 gap-4">
      {([{value:"EXPLORATORY_FULL_DATA",title:"探索数据关系",description:"使用全部可用数据进行科学探索。",icon:FlaskConical},{value:"TRAIN_ONLY_ML_SAFE",title:"为建模选择特征",description:"仅使用训练组数据，避免数据泄漏。",icon:ShieldCheck}] as const).map(m=><label key={m.value} className={`panel flex items-start gap-3 cursor-pointer ${mode===m.value?"!border-primary !bg-[#f3f7f6]":""}`}><input type="radio" name="analysis-mode" className="mt-1 accent-[#385c66]" checked={mode===m.value} onChange={()=>{setMode(m.value);setAnalysisId(null);dataset.reset();}}/><div><h3 className="flex gap-2 items-center"><m.icon size={17}/>{m.title}</h3><p className="text-sm muted mt-2">{m.description}</p></div></label>)}
    </div></fieldset>
    <div className="flex justify-between items-center mb-4"><h2>核心特征</h2><span className="text-sm muted">已选 {selected.length} 个</span></div><div className="grid md:grid-cols-3 gap-4">{core.map(f=><FeatureCard key={f.feature_name} feature={f} selected={selected.includes(f.feature_name)} toggle={()=>toggle(f.feature_name)}/>)}</div>
    <details className="mt-4 mb-8" data-testid="more-features"><summary>更多特征 · 共 {more.length} 个可用</summary><div className="grid md:grid-cols-3 gap-4 mt-3">{more.map((f,i)=><FeatureCard key={`${f.feature_name}-${f.gate_id}-${i}`} feature={f} selected={selected.includes(f.feature_name)} toggle={()=>toggle(f.feature_name)}/>)}</div></details>
    ...
    <Dialog open={action!==null} onOpenChange={open=>{if(!open)setAction(null);}}><DialogContent><DialogHeader><DialogTitle>{action==="dataset"?"用所选特征构建数据集":"确认分析请求"}</DialogTitle><DialogDescription>{batteryId} / {experimentId} · {mode==="TRAIN_ONLY_ML_SAFE"?"仅训练组选择":"探索性分析"}。此科学操作将使用下方所选输入。</DialogDescription></DialogHeader><ul className="list-disc pl-5">{selected.map(n=><li key={n}>{displayName(n)}</li>)}</ul><p className="text-sm muted">你的确认仅授权本次请求。不会发生自动特征选择或模型训练。</p>{(run.error||dataset.error) && <p role="alert">服务无法接受此请求，请检查前置条件后重试。</p>}<DialogFooter><Button variant="outline" onClick={()=>setAction(null)}>取消</Button><Button disabled={run.isPending||dataset.isPending} onClick={()=>action==="dataset"?dataset.mutate():run.mutate()}>确认请求</Button></DialogFooter></DialogContent></Dialog>
  </>;
}
