...
import { AssistantProvider, useAssistant } from "../../components/workbench/AssistantContext";
...
export function AssistantDrawer() {
  const { batteryId, experimentId } = useParams(); const location = useLocation(); const { open, setOpen, question, ask } = useAssistant();
  const page = navigation.find(n=>location.pathname.endsWith(n.path))?.label ?? "高级";
  return <Sheet open={open} onOpenChange={setOpen}><SheetTrigger asChild><Button variant="ghost" aria-label="Research Assistant"><MessageSquare/><span className="hidden sm:inline">Research Assistant</span></Button></SheetTrigger>
    <SheetContent className="sm:max-w-[420px] w-full"><SheetHeader><SheetTitle>科研助手</SheetTitle><SheetDescription>来自当前工作区的上下文。</SheetDescription></SheetHeader>
      <div className="py-8 space-y-8"><div className="panel"><p className="eyebrow">当前上下文</p><p className="font-medium">{batteryId} / {experimentId}</p><p className="muted text-sm mt-1">{page}</p></div>
        <div><h3 className="mb-3">可以探索的问题</h3><div className="space-y-2">{["这个实验能分析什么？", "为什么 TOF 不可用？", "有没有模型跑赢 Dummy？"].map(q=><Button key={q} variant="outline" className="w-full justify-start whitespace-normal h-auto py-3 text-left" onClick={()=>ask(q)}>{q}<ArrowUpRight className="ml-auto shrink-0"/></Button>)}</div></div>
